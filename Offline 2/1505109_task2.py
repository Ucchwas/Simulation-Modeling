...
class States:
    ...
    def update(self, sim, event):
        ...
        for i in range(sim.params.employee_count[3]) :
            if len(self.queue[3][i]) > self.max_cachier_q[i] :
                self.max_cachier_q[i] = len(self.queue[3][i])
    ...
    def finish(self, sim):
        ...
        print('Total customers arrived ',self.total_customer_number)
...
class ArrivalEvent(Event):
    ...
    def process(self, sim):
        ...
        if sim.states.server_busy_count[counter] > 0 and (counter == 0 or counter == 1):
            print('Food Queue')
            sim.states.queue[counter].append(self)

                # sim.states.people_in_q += 1
                # sim.states.queue.append(self.event_time)
        elif sim.states.server_busy_count[3] == sim.params.employee_count[3] and counter == 3:
            print('Cachier Queue')
            min_q = 0
            min_length = math.inf
            for i in range(sim.params.employee_count[3]):
...
class DepartureEvent(Event):
    ...
    def process(self, sim):
        ...
        if (len(sim.states.queue[counter]) == 0) and (counter == 1 or counter == 0) :
